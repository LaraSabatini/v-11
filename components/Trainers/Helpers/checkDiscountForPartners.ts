...
const checkDiscountForPartners = async (
  clientId: number,
  ...
  if (checkPayment.length) {
    ...
    const expirationDateCleaned = new Date(formatDescendingDate(expirationDate))
    const todayDate = new Date(`${year}-${month}-${day}`)

    if (expirationDateCleaned > todayDate) {
      for (let i = 0; i < lessonDates.length; i += 1) {
        const lessonDateCleaned = new Date(
          formatDescendingDate(lessonDates[i].date),
        )
        lessons.push({
          id: lessonDates[i].id,
...
export default checkDiscountForPartners
